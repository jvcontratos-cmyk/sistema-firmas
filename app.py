        ...
        try:
            ...
            for i, valor in enumerate(dnis_lima):
                if str(valor).strip() == dni_buscado:
                    tipo = sh_lima.cell(i + 1, 4).value or "Normal"
    ...
    try:
        ...
        for i, valor_celda in enumerate(dnis_en_excel):
            if str(valor_celda).strip() == dni_buscado:
                ...
                datos_fila = ["FIRMADO", fecha_fmt, tipo_detectado, nombre_trabajador, f_firma, f_foto]
...
def enviar_a_drive_script(ruta_archivo, nombre_archivo, folder_destino_id):
    try:
        ...
        payload = {"file": pdf_base64, "filename": nombre_archivo, "folderId": folder_destino_id}
...
def enviar_a_drive_script_retorna_url(ruta_archivo, nombre_archivo, folder_destino_id):
    try:
        ...
        payload = {"file": contenido_base64, "filename": nombre_archivo, "folderId": folder_destino_id}
...
if st.session_state['dni_validado'] is None:
        ...
        if sede_encontrada:
            ...
            if estado_sheet == "FIRMADO":
                ...
                if archivo_drive:
                    ...
                    st.error(f"**❌ CONTRATO NO UBICADO (VERIFIQUE QUE SU DNI ESTÉ CORRECTAMENTE ESCRITO).**")
        else:
    ...
    if st.session_state['firmado_ok']:
        st.balloons()
        st.markdown(f"""
            <div style="background-color: #f0fdf4; border-radius: 20px; padding: 40px; text-align: center; border: 2px solid #22c55e; margin-bottom: 25px;">
                <div style="font-size: 80px;">📝✅</div>
                <h1 style="color: #166534; margin-top: 10px; font-family: sans-serif;">¡PROCESO COMPLETADO!</h1>
                <p style="font-size: 18px; color: #374151;">Tu contrato ha sido firmado y guardado de forma segura.</p>
                <div style="background-color: white; border-radius: 10px; padding: 10px 20px; display: inline-block; border: 1px dashed #22c55e; margin-top: 10px;">
                    <span style="color: #6b7280; font-size: 12px; font-weight: bold;">CÓDIGO DE VALIDACIÓN:</span><br>
                    <code style="color: #065f46; font-weight: bold;">LDR-{st.session_state['dni_validado']}-{datetime.utcnow().strftime('%H%M%S')}</code>
                </div>
            </div>
        """, unsafe_allow_html=True)
        ...
        if os.path.exists(ruta_salida_firmado):
            with open(ruta_salida_firmado, "rb") as f:
                st.download_button("**📥 DESCARGAR CONTRATO FIRMADO**", f, file_name=f"FIRMADO_{nombre_archivo}", mime="application/pdf", type="primary", use_container_width=True)
    ...
    # === PANTALLA DE PROCESO (LECTURA + FOTO + FIRMA) ===
        ...
        # --- PASO 1: LECTURA ULTRA PRO (ZERO FLASH) ---
        ...
        # CONTENEDOR VACÍO PARA LA PANTALLA DE CARGA (Sin tintineo)
        ...
        if st.session_state['foto_bio'] is None:
            ...
            if foto_input is not None:
                ...
                placeholder_carga.markdown("""
                    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.95); z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                        ...
                        <h2 style="color: #333; margin-top: 20px;">VALIDANDO IDENTIDAD...</h2>
                    </div>
            ...
            with col_b:
                ...
                if st.button("🔄 Cambiar Foto"):
                    ...
                    placeholder_carga.markdown("""
                        <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255,255,255,0.95); z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                            <h2 style="color: #555;">REINICIANDO CÁMARA...</h2>
                        </div>
                    """, unsafe_allow_html=True)
                    st.session_state['foto_bio'] = None
                    st.rerun()
        ...
        if st.session_state['foto_bio'] is None:
            ...
            if enviar_firma:
                ...
                st.markdown("""
                <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(255, 255, 255, 0.9); backdrop-filter: blur(5px); z-index: 999999; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <div style="border: 10px solid #f3f3f3; border-top: 10px solid #FF4B4B; border-radius: 50%; width: 80px; height: 80px; animation: spin 1s linear infinite; margin-bottom: 20px;"></div>
                    <div style="font-size: 22px; font-weight: bold; color: #333; margin-bottom: 10px;">SUBIENDO ARCHIVOS...</div>
                    <div style="font-size: 18px; color: #d9534f; font-weight: bold;">⚠️ NO CIERRE LA VENTANA.</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                if canvas_result.image_data is not None:
                    ...
                    if img_data[:, :, 3].sum() == 0:
                        ...
                        try:
                            ...
                            # 🧠 CEREBRO DEL ROBOT 3.0 (DETECTA BANCO Y OTROS)
                            ...
                            if resp_pdf and resp_firma and resp_foto:
                                ...
                                if registro_ok:
                                    ...
                                    st.balloons()
                                    st.rerun()
                                else: st.error("❌ Falló el registro en Excel.")
                            else: st.error("❌ Error al subir archivos.")
